...
function queensPerformances() {
    ...
    let slay = currentCast.filter(function (queen) { return queen.performanceScore < 3; });
    let great = currentCast.filter(function (queen) { return queen.performanceScore >= 3 && queen.performanceScore < 10; });
    let good = currentCast.filter(function (queen) { return queen.performanceScore >= 10 && queen.performanceScore < 17; });
    let bad = currentCast.filter(function (queen) { return queen.performanceScore >= 17 && queen.performanceScore < 23; });
    let flop = currentCast.filter(function (queen) { return queen.performanceScore >= 23 && queen.performanceScore < 55; });
...
function runway() {
    ...
    let slay = currentCast.filter(function (queen) { return queen.runwayScore < 8; });
    let great = currentCast.filter(function (queen) { return queen.runwayScore >= 8 && queen.runwayScore < 18; });
    let good = currentCast.filter(function (queen) { return queen.runwayScore >= 18 && queen.runwayScore < 28; });
    let bad = currentCast.filter(function (queen) { return queen.runwayScore >= 28; });
    createRunwayDesc(slay, great, good, bad);
...
function lipSync() {
    ...
    if (thirdqueen && currentCast.length > 5) {
        ...
        if (score1 > 6 && score2 > 6 && randomNumber(0, 100) <= 99 && !doubleShantay && !noDouble) {
            if (randomNumber(0, 100) >= 95) {
                screen.createImage(bottomQueens[0].image, "tomato");
                screen.createBold(bottomQueens[0].getName() + ", shantay you stay.");
                screen.createImage(bottomQueens[1].image, "red");
        ...
        else if (score1 < 3 && score2 < 3 && randomNumber(0, 100) <= 1 && !doubleSashay && currentCast.length > 6 && noDouble == false) {
        ...
        else if (randomNumber(0, 1000) >= 9999 && disqOrDept == false) {
        ...
        else if (randomNumber(0, 1000) >= 9999 && !disqOrDept && bottomQueens.length == 2) {
...
class Queen {
    ...
    _calculateScores(min, max, stat = 0, queenName = "") {
        let score = randomNumber(min, max);
        let finalScore = score - (1.5 * stat)
        console.log(`Queen ${queenName}: Score = ${score} - (1.2 * ${stat}) = ${finalScore}`);
        return finalScore;
    }
    _calculateScoresLipsync(min, max, stat = 0, queenName = "") {
        let score = randomNumber(min, max);
        let finalScore = score - stat
        console.log(`Queen ${queenName}: Lipsync Score = ${score} - (${stat}) = ${finalScore}`);
        return finalScore;
    }    
    getName() {
        return this._name;
    ...
    getActing() {
        this.performanceScore = this._calculateScores(15, 35, this._actingStat, this._name);
    }
    getComedy() {
        this.performanceScore = this._calculateScores(15, 35, this._comedyStat, this._name);
    }
    getMarketing() {
        this.performanceScore = this._calculateScores(25, 45, this._comedyStat + this._brandingStat, this._name);
    }    
    getDance() {
        this.performanceScore = this._calculateScores(15, 35, this._danceStat, this._name);
    }
    getDesign() {
        this.performanceScore = this._calculateScores(15, 35, this._designStat, this._name);
    }
    getRunwayChallenge() {
        this.performanceScore = this._calculateScores(15, 35, this._runwayStat, this._name);
    }
    getImprov() {
        this.performanceScore = this._calculateScores(15, 35, this._improvStat, this._name);
    }
    //special 'gets':
    getSnatch() {
        this.performanceScore = this._calculateScores(25, 45, this._improvStat + this._comedyStat, this._name);
    }
    getRusical() {
        this.performanceScore = this._calculateScores(25, 45, this._danceStat + this._charismaStat, this._name);
    }
    getBall() {
        this.performanceScore = this._calculateScores(25, 45, this._designStat + this._runwayStat, this._name);
    }
    getRumix() {
        this.performanceScore = this._calculateScores(35, 55, this._danceStat + this._charismaStat + this._brandingStat, this._name);
    }
    getTalentShow() {
    ...
    getRunway() {
        this.runwayScore = this._calculateScores(12, 35, this._runwayStat, this._name);
    }
    getLipsync() {
        this.lipsyncScore = this._calculateScoresLipsync(0, this._lipsyncStat, this.unfavoritism, this._name) + this.favoritism;
    }
    getASLipsync() {
...
let argennis = new Queen("Argennis", 9, 7, 5, 10, 6, 9, 12, 7, 6, "Argennis");
let cristian = new Queen("Cristian Peralta", 9, 13, 11, 12, 10, 12, 11, 9, 10, "CristianP");
let gala = new Queen("Gala Varo", 9, 5, 5, 10, 9, 13, 15, 9, 14, "GalaVaro");
let kero = new Queen("Lady Kero", 7, 7, 7, 9, 9, 10, 11, 8, 8, "LadyKero");
let margaret = new Queen("Margaret Y Ya", 5, 6, 9, 9, 6, 9, 9, 10, 9, "MargaretYYa");
let matraka = new Queen("Matraka", 10, 8, 10, 10, 14, 14, 11, 14, 15, "Matraka");
let vallarta = new Queen("Miss Vallarta", 4, 4, 4, 4, 4, 4, 4, 3, 4, "MissVallarta");
let pixiePixie = new Queen("Pixie Pixie", 8, 8, 6, 8, 6, 10, 8, 8, 7, "PixiePixie");
let regina = new Queen("Regina Voce", 11, 10, 11, 11, 8, 7, 10, 16, 11, "ReginaVoce");
let serenaM = new Queen("Serena Morena", 6, 6, 5, 6, 6, 8, 8, 6, 5, "SerenaMorena");
let vermelha = new Queen("Vermelha Noir", 4, 4, 4, 5, 4, 7, 5, 4, 3, "VermelhaNoir");
let drmx = [argennis, cristian, gala, kero, margaret, matraka, vallarta, pixiePixie, regina, serenaM, vermelha];

//acting, improv, comedy, dance, design, runway, lipsync, branding, charismalet 
let yvie = new Queen("Yvie Oddly",12,10,5,14,11,12,15,11,12,"YvieS11");
let brooke = new Queen("Brooke Lynn Hytes", 9,11,5,11,15,15,15,9,12,"Brooke");
let akeria = new Queen("A'keria Chanel Davenport",9,16,8,14,11,11,11,10,11,"AkeriaS11");
let silky = new Queen("Silky Nutmeg Ganache", 10,12,14,10,6,8,10,12,15,"SilkyS11");
let vanjies11 = new Queen("Vanessa Vanjie Matteo",9,7,9,5,7,8,10,10,7,"VanessaS11");
...
//acting, improv, comedy, dance, design, runway, lipsync, branding, charisma 
let jaida = new Queen("Jaida Essence Hall", 9, 13, 9, 10, 12, 18, 13, 11, 11, "JaidaS12");
let crystalm = new Queen("Crystal Methyd", 8, 10, 10, 10, 8, 9, 8, 13, 10,"Crystal");
let gigi = new Queen("Gigi Goode", 12, 10, 13, 10, 18, 13, 10, 10, 15,"Gigi");
let jackie = new Queen("Jackie Cox", 10, 10, 10, 8, 7, 10, 11, 10, 10, "Jackie");
let heidi = new Queen("Heidi N Closet", 9, 8, 8, 9, 8, 8, 14, 10, 9, "Heidi");
let widow = new Queen("Widow Von'Du", 9, 7, 8, 12, 9, 10, 14, 8, 15, "Widow");
let jan = new Queen("Jan", 8, 8, 8, 10, 9, 10, 8, 8, 14,"JanS12");
...
let jaidynn = new Queen("Jaidynn Diore Fierce",7,7,8,9,7,7,13,8,10,"Jaidynn");
...
let kennedy = new Queen("Kennedy Davenport",8,13,12,13,7,7,15,10,10,"KennedyS7");
let max = new Queen("Max",14,8,10,7,8,12,3,12,9,"Max");
...
let violet = new Queen("Violet Chachki",9,9,8,10,17,15,7,10,10,"Violet");
...
function queensConjoined(wass = "") { 
    if (wass) {
        ...
        let great = currentCast.filter(function (queen) { return queen.performanceScore >= 8 && queen.performanceScore < 18; });
        let good = currentCast.filter(function (queen) { return queen.performanceScore >= 18 && queen.performanceScore < 28; });
        let bad = currentCast.filter(function (queen) { return queen.performanceScore >= 28 && queen.performanceScore < 33; });
        let flop = currentCast.filter(function (queen) { return queen.performanceScore >= 33 && queen.performanceScore < 55; });
        createPerformanceDesc(slay, great, good, bad, flop);
        makeoverCounter = true;
        screen.createHorizontalLine();
